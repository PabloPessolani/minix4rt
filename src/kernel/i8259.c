...
#ifdef	MRT
  /* Initialize counters, indexes, etc and the table of RT-IRQ handlers. */
  MRT_sv.counter.interrupts	= 0;
  MRT_si.soft_use   		= 0;

  for (i = 0; i < (NR_IRQ_VECTORS+NR_IRQ_SOFT) ; i++)
       {
	 MRT_si.irqtab[i].irq	 	= i;			/* Initialize irq descriptor indexes*/
	 MRT_irqd_rst(irqd_addr(i));				/* Reset interrupt descriptors	*/
 	 }
 #endif	/* MRT */
...
#ifdef MRT 
  d = irqd_addr(irq);
  if( d->irqtype != MRT_RTIRQ)
	{
	d->irqtype  	= MRT_NRTIRQ;	
	d->nrthandler  	= handler;
	d->period 		= 1;
	d->task 		= HARDWARE;
	d->watchdog		= HARDWARE;

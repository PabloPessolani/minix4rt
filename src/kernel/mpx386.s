...
.extern	_rtk_call
.extern	_MRT_idle
.extern     _MRT_flush_int
.extern     _MRT_IRQ_dispatch
.extern	_lock
.extern	_unlock
...
#ifdef MRT
.extern _MRT_sv 	! address of MRT_sv is equivalent to MRT_sv.flags
.extern _MRT_si	! address of MRT_si is equivalent to MRT_si.irqQ.bitmap
.extern _MRT_st	! address of MRT_st is equivalent to MRT_st.expQ.bitmap 
#define MRT_flags 	_MRT_sv
#define MRT_VTbitmap 	_MRT_st
#define MRT_IRQbitmap 	_MRT_si
#endif
.sect .text
!*===========================================================================*
!*				MINIX					     *
...
#define hwint_master(irq)	\
	...
	test (MRT_flags), MRT_RTMODE	;\
	jz 1f				;\
      push	irq			/* irq				  */;\
	call _MRT_IRQ_dispatch	;\
	pop ecx			;\
	jmp 0f			;\
1:	inb	INT_CTLMASK		;\
	orb	al, [1<<irq]	;\
...
#ifdef MRT
	cmp   ecx, RTKCALL! check for a RT Kernel Call 
	jne   std_call    ! is it a Standard Call ?
	push	ebx		! pointer to function parameters
	push	eax		! rtk_nr
	push	ecx		! RTKCALL
	call	_rtk_call	! rtk_call(RTKCALL, rtk_nr, parm_ptr)
				! caller is now explicitly in proc_ptr
	mov	AXREG(esi), eax ! put the rcode into the process stack
;	test (MRT_flags), MRT_NEEDSCHED ! test if rescheduling is needed
;	jz endcall		! do not call the scheduler
;     call _lock_pick_proc	! call the scheduler
	jmp   endcall	
std_call: 
#endif
...
#ifdef MRT
	test (MRT_flags), MRT_RTMODE	! do fast test to usually avoid function call
	jz no_flush1
	test (MRT_flags), MRT_FLUSHLCK 
	jnz no_flush1
	cmp (MRT_IRQbitmap), 0	! do fast test to usually avoid function call
	jnz   flush1
	cmp (MRT_VTbitmap), 0	! do fast test to usually avoid function call
	jz   no_flush1
flush1:
	call _MRT_flush_int
no_flush1:
...
#ifdef MRT
	jmp	_MRT_idle	! a "hlt" before this fails in protected mode
#else

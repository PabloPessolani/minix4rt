...
if(rp->p_nr == FLOPPY) printf("BLOCKED ");
	return;
  }
...
if(rp->p_nr == FLOPPY) printf("READY ");
...
#ifdef MRT
  if( MRT_sv.flags & MRT_RTMODE)
	{
	if (isuserp(caller_ptr) )
		{
 dest_ptr = proc_addr(dest);	/* pointer to destination's proc entry */

		if ( !issysmrt(dest) ) return(E_BAD_DEST);
		if (	MRT_is_realtime(caller_ptr))
			if (dest != MRTTASK )
				printf("mini_send(): a RT-process(%d) is "
					 "calling MINIX system call (dest PID)%d\n", caller_ptr->p_pid
					,dest_ptr->p_pid);
		}
	}
  else /* Non RT Mode */
	{
	if (isuserp(caller_ptr) && !issysmrt(dest)) return(E_BAD_DEST);
	}
#else

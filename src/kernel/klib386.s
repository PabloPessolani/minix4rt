...
#ifdef MRT
.define	_MRT_lock		! disable interrupts
.define	_MRT_unlock		! enable interrupts
.define	_MRT_saveFlock	! Save CPU Flags Register and disable interrupts
.define	_MRT_saveFunlock	! Save CPU Flags Register and enable interrupts
.define	_MRT_restoreF	! Restore CPU Flags Register
.define	_MRT_enable_irq	! enable an irq at the 8259 controller
.define	_MRT_disable_irq	! disable an irq
.define	_MRT_set_PIC	! set the PIC with a complete MASK
.define	_MRT_get_PIC	! get the PIC MASK
.define	_MRT_enable_PIC	! Enable PIC to receive new IRQs after receiving one
#else
...
#ifdef MRT
.extern _MRT_si	! address of MRT_si is equivalent to MRT_si.hard_use
#define MRT_HARD_USE	_MRT_si
#else
.extern     _irq_use
#endif
.extern	_blank_color
...
#ifdef MRT
	mov	eax, (MRT_HARD_USE) ! map of in-use IRQ`s
#else
...
!*				MRT_set_PIC 						    *
!*==========================================================================*/
! PUBLIC void MRT_set_PIC(unsigned mask)
! Set the PIC interrupt mask

	.align	16

_MRT_set_PIC:
	mov	eax, 4(esp)		! mask, ah=MSB, al=LSB
	pushf
	cli				
	outb	INT_CTLMASK		! set mask at master 8259
	movb	al, ah
	outb	INT2_CTLMASK	! set mask at slave 8259
	popf
	ret
...
!*				MRT_get_PIC 						    *
!*==========================================================================*/
! PUBLIC unsigned MRT_get_PIC( void)
! Get the PIC interrupt mask
	.align	16
_MRT_get_PIC:
	pushf
	cli				
	inb	INT2_CTLMASK! get mask at slave 8259
	movb	ah, al
	inb	INT_CTLMASK ! get mask at master 8259
	popf
	ret
...
!*				MRT_enable_PIC 						    *
!*==========================================================================*/
! PUBLIC unsigned MRT_enable_PIC(irq)
	.align	16
_MRT_enable_PIC:
	mov	eax, 4(esp)		! irq, ah=MSB, al=LSB
	pushf
	cli
	movb   ah, al		! ah = al
	movb	al, ENABLE		

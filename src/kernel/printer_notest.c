...
int irq;
{
 ...
 int psoft; 
	...
	prtd.rthandler = pr_soft;
	prtd.priority  = MRT_PRIHIGHEST + 1;
  	prtd.task      = PRINTER;
	strncpy(prtd.name, "PRINTER-SOFT",15);
 if ((psoft = MRT_set_softirq(&prtd)) < 0)
	panic("lp_handler: could not set PRINTER SOFT IRQ",psoft);
#endif /* MRT_TEST3 */

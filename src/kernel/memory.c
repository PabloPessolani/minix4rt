...
/* Set parameters for one of the RAM disks. */
...
#ifdef MRT
  static struct mrtinfo mrtinfo = { NR_VTIMERS, 
						MRT_NR_PRTY,
						NR_IRQ_VECTORS,
						NR_IRQ_SOFT,
						NR_MSGQ,
  ...
  case MIOCGPSINFO:
	...
	break;
  case MIOCGMRTINFO:
	/* The mrtstat program wants several system table addresses. */
	mrtinfo_phys = numap(m_ptr->PROC_NR, (vir_bytes) m_ptr->ADDRESS,
							sizeof(mrtinfo));
	if (mrtinfo_phys == 0) return(EFAULT);
	phys_copy(vir2phys(&mrtinfo), mrtinfo_phys, (phys_bytes) sizeof(mrtinfo));
	break;
  default:

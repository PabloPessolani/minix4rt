...
#ifdef MRT
 MRT_irqd_t mnxd,tmrd; 

 MRT_timerQ_init();					/* initialize the timerQ of vtimer 		*/
 MRT_vtimer_init();					/* initialize all vtimer descriptors 	*/
 MRT_vtimer_MINIX(1);
 MRT_vtimer_IDLE(MRT_PRILOWEST);

 /* initializing TRUE Timer Interrupt Descriptor */
 	tmrd.rthandler 	= MRT_clock_handler;	/* The RT-handler					*/
 	tmrd.nrthandler	= clock_handler;		/* The NRT-handler				*/
 	tmrd.task 		= CLOCK;			/* The task number to send an INT msg 	*/
 	tmrd.irqtype  	= MRT_RTIRQ;		/* The IRQ type = Event Driven	*/
 	tmrd.priority 	= MRT_PRIHIGHEST;		/* The IRQ priority = HIGHEST			*/
 	strncpy(tmrd.name, "RT-CLOCK",15);		/* The name of the descriptor			*/
 	MRT_irqd_set(CLOCK_IRQ, &tmrd);		/* Set the descriptor				*/
#endif

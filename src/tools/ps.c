...
#ifdef MRT
#define MRT_HDRATTR "PROC -PID- FLAGS PRTY BASE PERIOD LIMIT DEAD WDOG CMD\n"
#define MRT_FMTATTR "%4d %5d %5X %4d %4d %6d %5d %4d %4d %s\n"
#define MRT_HDRSTAT "PROC -PID- SCHEDS MDL TSTAMP MAXLAT MINLAX SENT RCVD CMD\n"
#define MRT_FMTSTAT "%4d %5d %6d %3d %6d %6d %6d %4d %4d %s\n"
#endif
...
#ifdef MRT
  	int 		mrt_flags;		/* Real Time Flags				*/
	priority_t	mrt_priority;	/* EFECTIVE Real Time priority		*/
	priority_t	mrt_baseprty;	/* BASE Real Time priority			*/
  	lcounter_t	mrt_period;		/* period in RT-ticks				*/
	scounter_t	mrt_limit;		/* maximun number of process schedulings  */
  	lcounter_t	mrt_deadline;  	 /* process deadline				*/
	pid_t 	mrt_watchdog;   	/* Watchdog process				*/

  	lcounter_t	mrt_scheds;		/* number of RT schedules			*/
  	lcounter_t	mrt_mdl;		/* Missed DeadLines				*/
  	lcounter_t	mrt_timestamp;	/* Last schedule timestamp in ticks 	*/
  	lcounter_t	mrt_maxlat;		/* Maximun latency in timer Hz 		*/
  	lcounter_t	mrt_minlax;		/* Minimun laxity in timer Hz 		*/
  	lcounter_t	mrt_msgsent;	/* messages sent by the process		*/
  	lcounter_t	mrt_msgrcvd;	/* messages received by the process 	*/
#endif
};
...
char *argv[];
{
...
#ifdef MRT
  int opt_MRTattr = FALSE;	/* -A */
  int opt_MRTstat = FALSE;	/* -S */
#endif
  ...
  if( opt_MRTattr == TRUE )
	{
	printf("%s", MRT_HDRATTR);
	for (i = -nr_tasks; i < nr_procs; i++) 
		{
		if (pstat(i, &buf) != -1 &&
			(buf.ps_euid == uid || buf.ps_ruid == uid))
			{
			printf(MRT_FMTATTR,
			       i,
				 buf.ps_pid,
  ...
  else if(opt_MRTstat == TRUE )
	{
	printf("%s", MRT_HDRSTAT);
	for (i = -nr_tasks; i < nr_procs; i++) 
		{
		if (pstat(i, &buf) != -1 &&
			(buf.ps_euid == uid || buf.ps_ruid == uid))
...
#ifdef MRT
  bufp->mrt_flags 	= ps_proc[p_ki].rt.flags;
  bufp->mrt_priority 	= ps_proc[p_ki].rt.priority;
  bufp->mrt_baseprty 	= ps_proc[p_ki].rt.baseprty;
  bufp->mrt_period 	= ps_proc[p_ki].rt.period;
  bufp->mrt_limit 	= ps_proc[p_ki].rt.limit;
  bufp->mrt_deadline 	= ps_proc[p_ki].rt.deadline;
  bufp->mrt_watchdog 	= ps_proc[p_ki].rt.watchdog;

  bufp->mrt_scheds 	= ps_proc[p_ki].st.scheds;
  bufp->mrt_mdl	 	= ps_proc[p_ki].st.mdl;
  bufp->mrt_timestamp 	= ps_proc[p_ki].st.timestamp;
  bufp->mrt_maxlat	= ps_proc[p_ki].st.maxlat;
  bufp->mrt_minlax	= ps_proc[p_ki].st.minlax;
  bufp->mrt_msgsent 	= ps_proc[p_ki].st.msgsent;
  bufp->mrt_msgrcvd 	= ps_proc[p_ki].st.msgrcvd;
